...
const SAITGovernanceDashboard = () => {
  ...
  const [dashboardData, setDashboardData] = useState({
    ...
    aiFundReserve: 40000000,
    teamAllocation: 20000000,
    partnerAllocation: 10000000,
  ...
  useEffect(() => {
    const fetchBlockchainInfo = async () => {
      if (!isWeb3Configured()) {
        console.log('Web3 not configured, using synthetic data only');
        return;
      }
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Merge blockchain data with synthetic data
        let mergedData = { ...syntheticData };
        
        if (blockchainData) {
          // Use blockchain data for key metrics when available
          mergedData = {
            ...syntheticData,
            totalSupply: blockchainData.totalSupply || syntheticData.totalSupply,
            saitCirculating: blockchainData.saitCirculating || syntheticData.saitCirculating,
            saitTreasury: blockchainData.saitTreasury || syntheticData.saitTreasury,
            satTreasury: blockchainData.satTreasury || syntheticData.satTreasury,
            aiFundReserve: blockchainData.aiFundReserve || syntheticData.aiFundReserve,
            teamAllocation: blockchainData.teamAllocation || syntheticData.teamAllocation,
            partnerAllocation: blockchainData.partnerAllocation || syntheticData.partnerAllocation,
            // Keep synthetic price data for now (until oracle integration)
            saitPrice: syntheticData.saitPrice,
            satPrice: syntheticData.satPrice,
            buybackRate: syntheticData.buybackRate,
            dataSource: 'hybrid',
            lastBlockchainUpdate: blockchainData.lastUpdated
          };
        } else {
          mergedData.dataSource = 'synthetic';
        }

        setDashboardData(mergedData);
        generateHistoricalData(mergedData);
        generateProjections(mergedData);
        generateGrantData();
        setLoading(false);
      } catch (error) {
  ...
  const allocationData = [
    ...
    { name: 'AI Fund Reserve', value: dashboardData.aiFundReserve, color: '#f59e0b' },
    { name: 'Team/Partners', value: dashboardData.teamAllocation + dashboardData.partnerAllocation, color: '#8b5cf6' }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              <div className="flex items-center gap-2 mb-2">
                <div className={`w-3 h-3 rounded-full ${blockchainData ? 'bg-green-400' : 'bg-yellow-400'} animate-pulse`}></div>
                <span className="text-sm">
                  {blockchainData ? 'Testnet Connected' : 'Synthetic Data'}
                </span>
              </div>
              {blockchainData && (
                <p className="text-xs text-blue-200">
                  Sepolia • Updated {new Date(blockchainData.lastUpdated).toLocaleTimeString()}
                </p>
              )}
              {blockchainError && (
                <p className="text-xs text-red-200">Error: {blockchainError}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto mb-6">
        <div className={`rounded-lg p-4 ${
          dashboardData.dataSource === 'hybrid' ? 'bg-blue-50 border-l-4 border-blue-500' :
          dashboardData.dataSource === 'blockchain' ? 'bg-green-50 border-l-4 border-green-500' :
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">
                {dashboardData.dataSource === 'hybrid' && 'Hybrid Mode: Combining blockchain + synthetic data'}
                {dashboardData.dataSource === 'blockchain' && 'Blockchain Mode: Live testnet data'}
                {dashboardData.dataSource === 'synthetic' && 'Synthetic Mode: Projected data from whitepaper'}
              </span>
            </div>
            {blockchainData && (
              <div className="text-xs text-gray-600">
                Supply: {(dashboardData.totalSupply / 1e6).toFixed(1)}M | 
                Circulating: {(dashboardData.saitCirculating / 1e6).toFixed(1)}M (Blockchain)
              </div>
            )}
          </div>
        </div>
